...
$(document).ready(function(){
    ...
    var card = 1;
    var sumproduk = 0;
    $.getJSON("../../assets/js/Data.json", function (jd) {
        // localStorage.removeItem("nama");
        var nama = localStorage.getItem("nama");
        // alert(nama);
        if (nama != null) {
            $("#In").append("My Profile");
            $("#In").attr("href","form_kelima");
            $("#ul2").append("<li class='nav-item'><a class='nav-link' href='form_awal' id='logout'>Log Out</a></li>");
        ...
        while (card <= 20) {
            ...
            if (ceek != true) {
                if (jd.gambar[i].nama == "Door Bumper") {
                    $("#product" + card + " a").attr("href", "form_kedua");
                }
	...
	// }
        ...
        $("#btnAddToCart").click(function () {
            window.location.href = "form_keempat";
...
function validasi(temp) {
    if (temp == true) {
        ...
        window.location.href = "form_awal";
    }
    else {
